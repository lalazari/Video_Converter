...
class Dispatcher(object):
    ...
    def dispatch(self):
        process_all_tasks(self.tasks, self.credentials)

    def image_transcode(self, img_path, path, data):
        self.tasks.append(image_transcode.s(img_path, path, data))

    def image_crop(self, img_path, path, base_name, data):
        self.tasks.append(image_crop.s(img_path, path, base_name, data))

    def image_resize(self, img_path, dir_v_path_out,
                     base_name, data):
        self.tasks.append(image_resize.s(img_path, dir_v_path_out, base_name, data))
    ...
    def audio_clip(self, au_path, dir_v_path_out, base_name, data):
        self.tasks.append(audio_clip.s(au_path, dir_v_path_out, base_name, data))
    ...
    def convert_video_crop(self, v_path, v_crop, dir_v_path_out, crop_path_out, data):
        self.tasks.append(convert_video_crop.s(v_path, v_crop, dir_v_path_out, crop_path_out, data))
    ...
    def video_rescale(self, v_path, dir_v_path_out, base_name, data):
        self.tasks.append(video_rescale.s(v_path, dir_v_path_out, base_name, data))
